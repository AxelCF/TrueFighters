            ...
            <div class="container flex justify-between mx-auto">
                <div class="w-full lg:w-8/12">
                    ...
                    @foreach ($posts as $post)
                        <div class="mt-6">
                        <div class="max-w-4xl px-10 py-6 mx-auto bg-indigo-900 rounded-lg shadow-md">
                            <div class="flex items-center justify-between">
                                <span class="font-light text-white">{{$post->created_at->format('d M Y') }}</span><a href="#"class="px-2 py-1 font-bold text-gray-100 bg-gray-600 rounded hover:bg-gray-500">{{ $post->category->name }}</a>
                            </div>
                            <div class="mt-2"><a href="#" class="text-2xl font-bold text-white hover:underline">{{ $post->title }}</a>
                            ...
                            <div class="flex items-center justify-between mt-4"><a href="#"
                                ...
                                <div>
                                    <a href="#" class="flex items-center"><img src={{ $post->user->profile_photo_path ?? 'https://ui-avatars.com/api/?name=?&color=7F9CF5&background=EBF4FF' }}
                                            alt="avatar" class="hidden object-cover w-10 h-10 mx-4 rounded-full sm:block">
                                        <h1 class="font-bold text-white hover:underline">{{ ucfirst($post->user->name) }}</h1>
                                    </a>
                                </div>
                            </div>
                        </div>
